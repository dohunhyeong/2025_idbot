...
| **역할** | Full-Stack Developer (AI Engineer) |
...
| # | 요구사항 | 기술적 과제 |
|---|----------|------------|
| 1 | 144종 법정감염병 최신 지침서 내용을 **100% 반영** | 문서 전처리 + 벡터 DB 설계 |
| 2 | 답변 시 공식 문서의 **출처 URL**을 반드시 표기 | 메타데이터 설계 |
| 3 | **할루시네이션 방지** — 제공 문서 범위 내에서만 답변 | 프롬프트 + RAG 구조 |
...
**초기 Fine-tuning 시도와 RAG 전환 결정**: 프로젝트 초반에는 도메인 특화 모델을 직접 학습시키는 Fine-tuning 방식을 검토했습니다. 그러나 Fine-tuning은 하드웨어 성능 요구사항이 높아 당시 환경에서 현실적으로 적용하기 어려웠습니다. 이에 외부 LLM과 전문 문서 검색을 결합하는 RAG 구조를 채택하는 것으로 방향을 전환했습니다.
...
144종 질병 × 10개 카테고리(질병개요, 증상, 치료 등) = **1,440개 독립 HTML 문서**로 분절했습니다.

**도메인 전문가 협업**: 10개 카테고리 구성은 감염병 전문가들과의 협의를 통해 결정했습니다. 단순히 기술적으로 분절하는 것이 아니라, 의료 현장에서 실제로 어떤 정보 단위로 검색이 이루어지는지를 전문가 의견을 반영하여 설계했습니다. 이렇게 도출된 10개 카테고리는 각 HTML 문서에 적용되어 검색 단위의 일관성과 의미 완결성을 확보했습니다.

각 문서는 Netlify를 통해 개별 URL을 부여받아, 챗봇이 답변 시 직접적인 웹 링크 출처를 제공하는 기반이 되었습니다.
...
- 누적 사용량 **3,000건** 돌파
- 단순 LLM(ChatGPT) 대비 Answer Relevancy **65% → 75%** (+10%p 향상)

### 2-7. 발견된 한계 — 2차년도 과제

운영을 거치면서 기술적 한계와 함께 코드 구조·운영 측면의 문제도 추가로 발견되었습니다.

| 구분 | 한계 | 원인 | 영향 |
|------|------|------|------|
| 기술 | **표 구조 손실** | `WebBaseLoader`가 HTML 표를 텍스트로 평면화 | 핵심 정보인 표 데이터가 컨텍스트에서 왜곡 → 답변 품질 저하 |
| 기술 | **동의어 미처리** | "신종플루" ↔ "신종인플루엔자 A" 등 매핑 부재 | 동의어 질문 시 엉뚱한 문서 검색 |
| 비용 | **API 비용 누적** | OpenAI API 종량제 의존 | 운영 비용 예측 불가, 장기 운영 부담 |
| 코드 | **유지보수 어려움** | `main.py`에 chain 로드·벡터 DB 로드 등 모든 로직 집중 | 기능 추가·수정 시 코드 파악 비용 증가 |
| 운영 | **관리자 조회 부재** | 별도 관리 인터페이스 없음 | 감염병관리지원단이 월별 누적 질문 수·내용 보고를 위해 DB에 직접 접근해야 하는 불편 발생 |

> 이 5가지 한계가 2차년도 프로젝트의 핵심 과제가 되었습니다.
...
| 코드 유지보수 | 서비스 객체화 — 기능별 서비스 클래스 분리 |
| 관리자 조회 | Admin 엔드포인트 및 Jinja2 대시보드 신설 |
...
| | FAISS × 12 | 도메인별 인덱스에서 유사 문서 병렬 비동기 검색 |
...
**Pre-Retrieval 단계**에서 불필요한 벡터 검색을 조기 차단하여 레이턴시를 낮추고, **Retrieval 단계**에서 12개 FAISS 인덱스를 `asyncio.gather()`로 병렬 비동기 검색하여 정확도를 높이며, **Post-Retrieval 단계**에서 결과를 병합·요약·출처 태깅하여 최종 답변을 생성합니다.
...
| 대안 | 문제점 | 결론 |
|------|--------|------|
| OpenAI API | 종량제 비용 — 운영 기간이 길어질수록 누적 비용 부담 | 제외 |
| HuggingFace 직접 로딩 | RTX 4090 + 64GB 메모리 환경에서 대용량 파라미터 파일 전체 로딩 시 무리 | 제외 |
| Ollama | 경량화된 로컬 서빙으로 비용 0원, 서버 사양 내 안정적 구동 | **채택** |

**exaone3.5:7.8b 선택 이유**

- 한국어 최적화 모델 — 의료·행정 도메인의 한국어 문서에 적합
- RTX 4090 환경에서 실험 결과 평균 응답 지연시간 **약 10초** — 챗봇 서비스로 허용 가능한 범위로 판단하여 채택
...
| **관리자 대시보드** | 감염병관리지원단이 기간별 누적 질문 수·내용을 토큰 인증으로 조회 (`GET /admin`) |
| **Langfuse 트레이싱** | RAG 파이프라인 각 단계의 레이턴시·토큰 사용량 모니터링 |

감염병관리지원단은 매달 누적 질문 수와 질문 내용을 보고해야 했습니다. 이를 위해 `X-ADMIN-TOKEN` 헤더 인증 기반의 관리자 전용 엔드포인트(`/admin`, `/admin/logs`, `/admin/logs/search`)를 신설하여 비개발자도 직접 데이터를 확인할 수 있도록 했습니다.
...
| **코드 구조** | main.py 단일 파일 집중 | 서비스 객체화 (10개 서비스 클래스) | 유지보수성 향상 |
...
- **멀티 리트리버 구조**: 12개 도메인별 FAISS 인덱스를 `asyncio.gather()`로 병렬 비동기 검색하여, 단일 인덱스 대비 검색 정확도를 높이면서도 레이턴시를 관리할 수 있었습니다.
- **Langfuse 도입**: RAG 파이프라인의 각 단계를 트레이싱하여 병목 구간을 정량적으로 파악하고 개선할 수 있는 기반을 마련했습니다.
- **1차 → 2차 서사 구조**: 1차년도에 발견한 문제를 2차년도에 체계적으로 해결하는 과정에서 기술적 깊이와 운영 인사이트를 모두 확보했습니다.

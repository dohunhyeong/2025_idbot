# Busan CIDC RAG Chatbot Backend

FastAPI 기반 Retrieval-Augmented Generation (RAG) 백엔드 서버

이 시스템은 법정 감염병 정보를 안내하는 챗봇의 백엔드이며 사용자 질문을
분석하여 벡터 검색과 LLM을 이용해 답변을 생성한다.

## 2024년 챗봇의 문제점(2024년 챗봇 프로젝트에 대한 자세한 내용은 @./2024_proj.md를 확인!)
1. 문서가 제대로 파싱되지 않아서, 구조가 제대로 반영되지 않은 줄글들로 벡터 DB가 구성되고, 이는 검색 정확도 하락과 LLM에게 전달되는 컨텍스트의 품질 또한 저하시켜 최종적으로 답변 정확도를 저해하는 요인이다.
2. 동의어에 대한 처리가 이루어지지 않아서, 신종인플루엔자라 물어보면 대답을 하는데, 이에 대한 동의어로 질문을 하면 엉뚱한 문서를 검색하는 문제가 발생함.
3. OpenAI의 API 기반의 LLM 을 활용하여 비용 문제가 발생함.

## 2025년 챗봇 구현 시 해결하고자 한 부분
1. Upstage Document-parser API를 활용하여 문서의 구조를 반영하여 파싱해오고자 하였습니다. 그리고 기존 법정감염병 알아보기 문서(주로 표의 형식으로 이루어짐)와 감염병 카테고리별 문서(법정감염병들을 수인성매개감염병, 생물테러감염병, 성매개감염병 등의 10개의 카테고리로 나누고, 해당 카테고리별 감염병들에 대한 세부 관리 지침 등 좀 더 자세한 내용을 다루는 문서들을 추가하였으며, 이 문서는 일반적인 문서와 유사하게 이루어져있다)를 별도로 전처리하였음 -> 이 부분에 대한 자세한 내용은 @.doc_preprocessing.md 파일을 참조하세요!
2. 동의어 사전(@../resources/metadata/disease_metadata.csv)을 구축하였음.
3. Ollama를 Linux 서버에 호스팅하고, exaone3.5:7.8b를 LLM 으로 활용하고, 임베딩 모델은 bge-m3 를 활용했어요.
4. 멀티리트리버 구조를 활용해서 정확한 검색을 하도록 함.

## 1차년도 진행 중 문제점
- Fine-tuning 을 하려 했으나, 하드웨어 성능의 문제로 인해서, RAG 구조를 도입함.

## 1차년도 수행 내용 추가
- 문서 분절 시, 도메인 전문가들과의 협업 진행.
- 도메인 전문가들이 질병별로 구분하기 적합하다고 생각하는 카테고리들에 대해서 조언해줌.
- 이를 반영하여 도메인 전문가들과의 협업으로 144개의 질병별 10개의 카테고리 문서를 생성하여 최종적으로 1440개의 문서를 생성함.

## 1차년도 프로젝트 수행 후 문제점
1. 객체화가 되지 않고, main.py 파일에 chain 로드, 벡터 데이터베이스 로드 등의 모든 코드들이 몰려있었음.
2. 감염병 관리자들이 데이터베이스 조회에 어려움을 겪음(매달 누적 조회수 보고를 해야했음).

## 1차년도 프로젝트에 수행 후 문제점에 대한 해결점
1. 코드 객체화를 통해 유지보수가 용이하도록 함.
2. Admin 엔드포인트를 추가해서, 관리자들이 별도로 접근하여 mongo DB의 내용을 확인할 수 있도록 하였음.

## 2차년도 수행내용추가
### Ollama 선택의 배경
- 기존에 ChatGPT 모델은 API 비용 소모가 심해서, 이를 오픈 소스 모델로 변경하여 비용을 절감하고자 하였음.
- 허깅페이스 모델을 로컬에서 불러와서 사용하기에는 현재 법정감염병 관리 지원단에서 임대한 서버의 사양이 RTX 4090, 64기가 메모리를 사용하고 있기 때문에 무리가 있다고 판단하였습니다.
- 그래서 허깅페이스 모델 파라미터 파일 전체를 불러와서 사용하기 보다는 ollama를 호스팅하여 사용하기로 결정했습니다.

### Exaone3.5:7.8b 모델 선택의 배경
- 한국어 언어 모델이라는 점.
- 현재 법정감염병 관리 지원단에서 임대한 서버에는 RTX4090 GPU, 64기가 메모리를 사용하고 있었기 때문에, 실험 결과 7.8b 모델을 사용하였을 때, 평균적으로 지연시간이 약 10초 걸리는 것을 확인하였고, 이에 해당 모델을 선택함.
------------------------------------------------------------------------

# System Overview (시스템 개요)

주요 기능

-   감염병 질의 응답
-   질병 동의어 정규화
-   감염병 급수 질문 처리
-   Intent 질문 처리 (인사 / 챗봇 정보 / 감염병 통계)
-   Multi Vector Store Retrieval
-   LLM 기반 Answer Generation
-   MongoDB Query Logging
-   React Frontend API 제공

------------------------------------------------------------------------

# Architecture Diagram (아키텍처 다이어그램)

``` mermaid
flowchart TD

User[User Question] --> API[FastAPI API /query]

API --> InputService
InputService --> IntentService

IntentService -->|Greeting / Meta / Stats| IntentResponse[Return Intent Response]

IntentService -->|Disease Question| NormalizationService

NormalizationService --> GradeService

GradeService -->|Grade Question| GradeResponse[Return Grade Answer]

GradeService -->|Normal Disease Query| RoutingService

RoutingService --> RetrieverLoader

RetrieverLoader --> MultiRetriever

MultiRetriever --> CommonRetriever
MultiRetriever --> BioterrorARetriever
MultiRetriever --> BioterrorBRetriever
MultiRetriever --> RespiratoryRetriever
MultiRetriever --> ZoonoticRetriever
MultiRetriever --> WaterFoodRetriever
MultiRetriever --> SexualBloodRetriever
MultiRetriever --> TickRetriever
MultiRetriever --> VaccineRetriever
MultiRetriever --> HealthcareRetriever
MultiRetriever --> TbRetriever
MultiRetriever --> EtcRetriever

CommonRetriever --> RetrievalResults
BioterrorARetriever --> RetrievalResults
BioterrorBRetriever --> RetrievalResults
RespiratoryRetriever --> RetrievalResults
ZoonoticRetriever --> RetrievalResults
WaterFoodRetriever --> RetrievalResults
SexualBloodRetriever --> RetrievalResults
TickRetriever --> RetrievalResults
VaccineRetriever --> RetrievalResults
HealthcareRetriever --> RetrievalResults
TbRetriever --> RetrievalResults
EtcRetriever --> RetrievalResults

RetrievalResults --> Aggregator
Aggregator --> Summarizer
Summarizer --> SourceService

SourceService --> FinalAnswer[Final Answer]

FinalAnswer --> MongoDB[(MongoDB Logging)]
```

------------------------------------------------------------------------

# System Architecture (전체 시스템 구조)

``` mermaid
flowchart LR

React[React Frontend] --> FastAPI[FastAPI Backend]
FastAPI --> Pipeline[RAG Pipeline]
Pipeline --> Retrieval[Retriever Layer]
Retrieval --> Embedding[Embedding Service]
Embedding --> VectorDB[(FAISS Vector Store)]
Pipeline --> LLM[LLM Answer Generation]
Pipeline --> Mongo[(MongoDB Logging)]
```

------------------------------------------------------------------------

# RAG Architecture (RAG 아키텍처)

이 시스템은 **Advanced RAG / Modular RAG** 구조를 채택한다.
단순히 쿼리를 벡터 검색에 그대로 넘기는 Naive RAG와 달리,
검색 전(Pre-Retrieval) · 검색(Retrieval) · 검색 후(Post-Retrieval) 3단계로 파이프라인이 구성된다.

## Pre-Retrieval

사용자 질문을 벡터 검색에 넘기기 전에 질문의 의도와 형태를 정제한다.

| 구성 요소 | 역할 |
|----------|------|
| IntentService | 질문 유형 분류 (인사 / 챗봇 정보 / 감염병 통계 / 질병 질의). 질병 질의가 아닌 경우 검색 없이 즉시 응답 |
| NormalizationService | disease_metadata.csv 기반 동의어·약칭을 정규 병명으로 변환하고 감염병 급수(1~4급) 설정 |
| GradeService | 급수 관련 질문 감지 시 벡터 검색 없이 즉시 응답 |
| RoutingService | 정규화된 쿼리의 키워드를 분석하여 검색할 Vector Store 목록 결정 |

## Retrieval

도메인별로 분리된 FAISS 인덱스에서 병렬로 검색을 수행한다.

| 구성 요소 | 역할 |
|----------|------|
| RetrieverLoader | 라우팅 결과에 따라 Retriever 클래스를 동적으로 로드 |
| Embedding Service | Ollama bge-m3 모델로 쿼리를 벡터로 변환 |
| FAISS Vector Store | 도메인별 12개 인덱스(common, bioterror_A, bioterror_B, respiratory 등)에서 유사 문서 병렬 검색 |

## Post-Retrieval

여러 Retriever에서 수집된 결과를 정제하고 최종 답변을 생성한다.

| 구성 요소 | 역할 |
|----------|------|
| AggregatorService | 복수 Retriever의 답변 병합 및 중복 제거 |
| SummarizerService | LLM(Ollama exaone3.5:7.8b)으로 병합 결과를 단일 답변으로 재요약 |
| SourceService | 답변에 출처 URL을 추가 |
| LoggingService | 질의·응답 전체를 MongoDB에 저장 |

------------------------------------------------------------------------

# Project Structure (프로젝트 구조)

    backend
    │
    ├ main.py
    │
    ├ app
    │ ├ api
    │ │ └ admin_router.py
    │ │
    │ ├ core
    │ │ ├ llm_service.py
    │ │ └ embedding_service.py
    │ │
    │ ├ models
    │ │ └ query.py
    │ │
    │ ├ pipeline
    │ │ └ pipeline.py
    │ │
    │ ├ services
    │ │ ├ input_service.py
    │ │ ├ intent_service.py
    │ │ ├ grade_service.py
    │ │ ├ normalization_service.py
    │ │ ├ routing_service.py
    │ │ ├ retriever_loader.py
    │ │ ├ aggregator_service.py
    │ │ ├ summarizer_service.py
    │ │ ├ source_service.py
    │ │ └ logging_service.py
    │ │
    │ └ retrievers
    │   ├ base_retriever.py
    │   ├ common.py
    │   ├ bioterror_A.py
    │   ├ bioterror_B.py
    │   ├ respiratory.py
    │   ├ zoonotic.py
    │   ├ water_food.py
    │   ├ sexual_blood.py
    │   ├ tick.py
    │   ├ vaccine.py
    │   ├ healthcare.py
    │   ├ tb.py
    │   └ etc.py
    │
    ├ infra
    │ └ mongodb
    │   ├ mongo_client.py
    │   └ query_log_repository.py
    │
    ├ resources
    │ ├ metadata
    │ │ └ disease_metadata.csv
    │ └ vectorstore
    │
    └ web
      ├ router.py
      └ templates
        └ admin.html

------------------------------------------------------------------------

# API Endpoints (API 엔드포인트)

## Query API

POST /query

Request (요청)

``` json
{
  "query": "탄저의 예방 방법은?"
}
```

Response (응답)

``` json
{
  "query_id": "...",
  "mongo_id": "...",
  "raw_query": "탄저의 예방 방법은?",
  "normalized_query": "탄저의 예방 방법은?",
  "grade": 1,
  "retrievers_used": ["common", "bioterror_A"],
  "latency_ms": 1800,
  "answer": "..."
}
```

------------------------------------------------------------------------

# Admin APIs (관리자 API)

## 관리자 대쉬보드를 만든 목적
- 감염병 관리 지원단 측에서 일정 기간별로 누적 질문 수, 질문 내용 등을 확인해보고자하는 니즈가 있었음.
- 이에 관리자 키로 MongoDB의 내용을 조회할 수 있도록 만듦.

관리자 대시보드

GET /admin

로그 조회

GET /admin/logs

로그 검색

GET /admin/logs/search?q=탄저

로그 상세 조회

GET /admin/logs/{mongo_id}

인증 헤더: `X-ADMIN-TOKEN: your_admin_token`

------------------------------------------------------------------------

# Environment Variables (환경 변수)

.env example

    MONGODB_URI=mongodb://localhost:27017
    MONGODB_DB=openai
    MONGODB_COLLECTION=2026cidc

    ADMIN_TOKEN=your_admin_token

------------------------------------------------------------------------

# Running the Server (서버 실행)

설치

    poetry install

실행

    poetry run uvicorn main:app --reload

서버 주소

    http://localhost:8000

------------------------------------------------------------------------

# Frontend (프론트엔드)

Frontend는 별도의 React 프로젝트에서 제공된다.

Busan Chatbot Frontend
- frontend에 대한 자세한 내용은 @./2025_frontend.md 파일을 참조.

기술 스택

-   React 19
-   Vite
-   React Router
-   React Markdown

Frontend URL

    http://localhost:5173

Backend API

    http://localhost:8000

------------------------------------------------------------------------
# 서버 배포
- 서버 배포에 관한 자세한 내용은 @./deployment_guide.md 파일을 참조할 것.
  
------------------------------------------------------------------------
# 최종 결과
- answer relevancy 약 5% 향상.
- faithfulness 약 5% 향상.

------------------------------------------------------------------------
# 요구사항
## 포트폴리오 제작을 위한 내용 구성
- 2024년 6월부터 2025년 2월까지 대략 2년에 걸쳐서 챗봇을 배포하고, 6개월 정도 운영을 해보았고, 총 3,000건 정도의 사용이 있었던 것을 확인했습니다. 그리고 2차년도(2025년 6월 시작)에는 챗봇의 성능 개선을 최종 목표로 하여 프로젝트를 진행하였습니다.
- 위의 내용을 잘 읽고, 필요한 파일들이 있으면 해당 경로에 있는 파일들을 읽어와서 추가 지식을 확보하세요. 이렇게 1차년도~2차년도에 이르기까지 제 프로젝트에 대한 포트폴리오를 구성해주면 좋겠어요.

- 형식: markdown
- 내용
: 프로젝트를 진행하면서 생겼던 문제점, 해결방안, 기술 스택, 프로젝트(프로그램, 시스템) 아키텍쳐 등등의 요소들을 포함해서 논리적이게끔 내용을 구성해주면 좋겠구요.
- 1차년도에 챗봇 만들 때 생긴 문제점과 당시 해결방안(@./2024_proj.md 참조), 1차년도에 운영을 하며 발견한 문제들을 해결하기 위한 2차년도의 해결방안(지금 이 파일과 @./doc_preprocessing.md 파일 참조)에 대한 내용들도 구성해줘요.

* 이외에도 포트폴리오 작성에 필요한 내용들이 있으면 나에게 물어봐요! 

